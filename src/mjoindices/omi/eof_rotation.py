...
The EOFs are first calculated in the original way specified in the file omi_calculator.py. The postprocessing
function is replaced by the following steps:
...
To use this method, run :func:'calc_eofs_from_olr_with_rotation' using a suitable OLR dataset. The PCs can then 
be computed using the original OMI calculation :func:'calculate_pcs_from_olr' in omi_calculator.py.
...
def post_process_rotation(eofdata: eof.EOFDataForAllDOYs, sign_doy1reference: bool = True,
    ...
    See documentation of  the methods :meth:`correct_spontaneous_sign_changes_in_eof_series` in omi_calculator.py
